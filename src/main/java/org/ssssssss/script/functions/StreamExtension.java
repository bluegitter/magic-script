...
public class StreamExtension {
	...
	public Object map(Object target, @Comment("转换函数，如提取属性(item)=>item.xxx") Function<Object[], Object> function) {
		List<Object> objects = arrayLikeToList(target);
	...
	public Object filter(Object target, @Comment("过滤条件，如(item)=>item.xxx == 1") Function<Object[], Object> function) {
		List<Object> objects = arrayLikeToList(target);
		List<Object> results = new ArrayList<>(objects.size());
		for (int i = 0, len = objects.size(); i < len; i++) {
			Object object = objects.get(i);
	...
	public Object each(Object target, @Comment("循环函数，如循环添加属性(item)=>{item.xxx = 'newVal'}") Function<Object[], Object> function) {
		List<Object> objects = arrayLikeToList(target);
		List<Object> results = new ArrayList<>(objects.size());
		for (int i = 0, len = objects.size(); i < len; i++) {
			Object object = objects.get(i);
	...
	public Object sort(Object target, @Comment("排序函数，如从大到小(a,b)=>a-b") Function<Object[], Object> function) {
		List<Object> objects = arrayLikeToList(target);
	 ...
	 */
	@Comment("对集合进行分组")
	public Map<Object, List<Object>> group(Object target, @Comment("分组条件，如item=>item.xxx + '_' + item.yyy") Function<Object[], Object> condition) {
		return arrayLikeToList(target).stream()
	 ...
	 */
	@Comment("将两个集合关联起来")
	public List<Object> join(Object source, @Comment("另一个集合") Object target, @Comment("关联条件，如:(left,right)=>left.xxx = right.xxx") Function<Object[], Object> condition) {
	...
	public Object limit(Object source, @Comment("跳过的数量") int value) {
		return toOriginType(source, arrayLikeToList(source).stream().limit(value).collect(Collectors.toList()));
	}

	@Comment("判断集合是否都满足条件")
	public boolean every(Object source, @Comment("判断条件") Function<Object[], Object> condition) {
		List<Object> objects = arrayLikeToList(source);
	...
	public boolean some(Object source, @Comment("判断条件") Function<Object[], Object> condition) {
		List<Object> objects = arrayLikeToList(source);
		for (int i = 0, size = objects.size(); i < size; i++) {
			if (BooleanLiteral.isTrue(condition.apply(new Object[]{objects.get(i), i}))) {
				return true;
	...
	public Object reduce(Object source, @Comment("处理函数，如累加计算：(val,item)=>val + item") Function<Object[], Object> reduceFunction) {
		List<Object> objects = arrayLikeToList(source);
		if (objects.isEmpty()) {
			return null;
		}
